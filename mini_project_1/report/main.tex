...
Project files can be found in this GitHub repository: \url{https://github.com/mbelanger0/Eclectronics_SP26/tree/main/mini_project_1}

\section*{Design Decisions}

\subsection*{Oscillator Component Value Selection}

Designing this oscillation circuit required picking values for $V_{ref}$ and the various feedback resistors and capacitors
as seen in the diagram fo the hysteretic oscillator circuit below.\\
 
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.3\textwidth]{assets/oscillator_schematic.png}
  \caption{Overall op-amp hysteretic oscillator}
  \label{fig:oscillator_schematic}
\end{figure}
...
The $R`$ resistor values I chose were $1k\Omega$ and this was to match other resistors on the board,
like the LED current limiting resistor.
...
the op-amp MCP6022 IC uses a $0.1\mu F$ capacitor for its power bypass capacitance meaning
...
has a 10\% tolerance. This makes for less variation in the output frequency due to tolerances.
...
than the nominal $4.55M\Omega$. Using four resistors does give more opportunity for
tolerances to become an issue but it puts the equivalent resistance very close to
the nominal.\\
...
matching the $R`$ resistor values.
...
As such a $0.1\mu F$ cap is connected less than 2mm away from the power pin and the $1\mu F$ bypasss
capacitor on the output of the linear regular is within 100mm of the op-amp and acts as bulk capacitance
for the op-amp.

\subsection*{Worse Case Analysis}

I did an initial simulation in LTSpice to verify the nominal behavior. The output
behavior shows a frequency of 1Hz as expected. Schematic and simulation output can be seen below:

 \begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{assets/nominal_sim.png}
  \caption{LTSpice schematic used for nominal behavior simulation}
  \label{fig:nominal_sim}
\end{figure}

I also used LTSpice to do a worst-case analysis on the circuit. The simulation includes tolerance for
...
The LTSpice schematic can be seen below:
 ...
 \begin{figure}[htbp]
  ...
  \caption{LTSpice schematic used for worst-case simulation}
...
were 2029 combination. Out of these, the highest frequency was 1.091Hz and the lowest was 0.935Hz
both of which are within the 1Hz $\pm$ 10\% requirement.\\
...
\begin{figure}[htbp]
  ...
  \caption{Distribution of frequencies in worst case simulation}
...
means that the bypass capacitor on the input of the linear regulator is not included. Given the nature
of the bypass capacitor though, the behavior of the oscillator should\\

The simulation also does not include tolerance for the current limiting resistor for the LED as suggested.
I did choose to include the bypass cap for the op-amp and the output of the 3.3V voltage supply but these
should not influence the oscillation behavior much.

\subsection*{Layout Considerations}
I followed the convention of having 8mil trace widths for signals and 20mil power trace widths where possible.
...
wanted to try to follow signal integrity best practices. I tried to keep power traces short and signals traces also short and away
...
pad bridging to another so the extra spacing should be helpful. I also wanted to keep sufficient space for component references.
Although also potentially unnecessary, I have found them helpful when assembling boards if you don't have access to the layout.\\

The ERC in the layout does indicate errors due to the USB connector. The footprint includes the full outline of the port but 
obviously the board can't be extended that far as it would then be impossible to plug the port into a USB port. Therefore I am ignoring
those errors. I edited the footprint to remove part of the footprint that extends beyond the board.
